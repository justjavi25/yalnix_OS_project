...
//
// Mar 2024: if in kernel, print out stats at 0

// Jan 2025: force abort on serious kernel heap corruption
...
void
sanity_fail(char *msg) {

  #ifdef IN_KERNEL
  TracePrintf(0,"Something that happened since the last `Kernel heap OK' message has corrupted the kernel heap.\n");
  TracePrintf(0,"The framework will halt and dump core, so you can find the problem.\n");
  helper_abort(msg);
  #else
  helper_maybort(msg);
  #endif
...
#ifdef IN_KERNEL
  static int prev_dirty = 0;
  static int prev_bytes = 0;
  
  if ( (prev_dirty != dirty) || (prev_bytes != used_block_bytes) ) {
    TracePrintf(0,"Kernel heap OK.\n");
    TracePrintf(0, "Kernel heap usage: %d blocks allocated, %d bytes total\n", dirty, used_block_bytes);
    prev_dirty = dirty;
    prev_bytes = used_block_bytes;
  }
